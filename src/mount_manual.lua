...
-- ===== Row helper (tanpa UIListLayout di level row) =====
local function newRow(height)
  ...
  left.Position=UDim2.fromOffset(10,8)
  left.Parent=row

  local right=Instance.new("Frame")
  right.BackgroundTransparency=1
  right.Size=UDim2.new(1,-(150+20),1,0) -- sisa lebar
  right.Position=UDim2.fromOffset(150+10,0)
  right.Parent=row
...
local function loadWP()
  ...
  if typeof(data)=="table" then for _,v in ipairs(data) do if typeof(v)=="table" and v.x and v.y and v.z then table.insert(waypoints, Vector3.new(v.x,v.y,v.z)) end end end
end
