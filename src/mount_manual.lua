...
-- Section "Manual": Waypoints + Delay + Auto Respawn/Kill + Gerak 3x/8stud + Auto Rejoin + Auto Loop
-- TIDAK membuat GUI baru. Hanya mengisi section "Manual" di Tab Mount.

-- ===== Ambil container dari UI utama =====
local UI = _G.danuu_hub_ui
if not UI or not UI.Tabs or not UI.Tabs.Mount then return end
local container = (UI.MountSections and UI.MountSections["Manual"])
...
-- ===== Mini UI helpers (match tema hub) =====
local Theme = {
...
-- ===== Layout helper: satu bar (kiri label, kanan kontrol) ditempel ke container =====
local function newRow(height)
  local row=Instance.new("Frame")
  row.BackgroundColor3=Theme.card
  row.Size=UDim2.new(1,-4,0,height or 54)
  ...
  left.Font=Enum.Font.GothamSemibold; left.TextSize=16; left.TextXAlignment=Enum.TextXAlignment.Left; left.TextColor3=Theme.text; left.Text=""; left.Parent=row

  local right=Instance.new("Frame"); right.BackgroundTransparency=1; right.Size=UDim2.new(1,-150,1,0); right.Parent=row
  local rlay=Instance.new("UIListLayout",right); rlay.FillDirection=Enum.FillDirection.Horizontal; rlay.Padding=UDim.new(0,10)
...
-- ===== BLOCK: List Waypoints (card + tombol TP/Delete) =====
do
  -- judul kecil dalam section (label biasa, bukan section baru)
  local t = Instance.new("TextLabel")
  t.BackgroundTransparency=1; t.TextXAlignment=Enum.TextXAlignment.Left
  t.Text="List Waypoints"; t.Font=Enum.Font.GothamBlack; t.TextSize=16; t.TextColor3=Theme.text
  t.Size=UDim2.new(1,0,0,22); t.Parent=container

  local card=Instance.new("Frame"); card.BackgroundColor3=Theme.bg; card.Size=UDim2.new(1,-4,0,150)
  card.Position=UDim2.fromOffset(2,0); card.Parent=container
  ...
  local scroll=Instance.new("ScrollingFrame"); scroll.BackgroundTransparency=1; scroll.Size=UDim2.fromScale(1,1)
  scroll.ScrollBarThickness=6; scroll.CanvasSize=UDim2.new(0,0,0,0); scroll.Parent=card
  ...
  local function refresh()
    ...
    for i,pos in ipairs(waypoints) do
      local row=Instance.new("Frame"); row.BackgroundColor3=Theme.card; row.Size=UDim2.new(1,0,0,30); row.Parent=scroll
      ...
      del.MouseButton1Click:Connect(function()
        table.remove(waypoints,i); refresh(); saveWP()
      end)
    end
  end
  container._refreshWP = refresh

  -- Row tombol Set/Delete
  local _, L, R = newRow(40); L.Text = "Waypoints"
  ...
  btnSet.BackgroundColor3=Theme.accA; corner(btnSet,8); stroke(btnSet,Theme.accB,1).Transparency=.35; btnSet.Parent=R
  ...
  btnDel.BackgroundColor3=Theme.card; corner(btnDel,8); stroke(btnDel,Theme.accA,1).Transparency=.5; btnDel.Parent=R
  ...
  btnDel.MouseButton1Click:Connect(function()
    if #waypoints>0 then table.remove(waypoints,#waypoints); refresh(); saveWP() end
  end)
...
-- ===== Row: Delay + Auto Respawn/Kill =====
local _, dL, dR = newRow(46); dL.Text = "Delay"
...
local _, oL, oR = newRow(46); oL.Text="Opsi"
...
danceBtn.BackgroundColor3=Settings.moveDance and Theme.accA or Theme.card; corner(danceBtn,8); stroke(danceBtn,Theme.accA,1).Transparency=.45; danceBtn.Parent=oR
...
rjBtn.BackgroundColor3=Settings.autoRJ and Theme.accA or Theme.card; corner(rjBtn,8); stroke(rjBtn,Theme.accA,1).Transparency=.45; rjBtn.Parent=oR
...
corner(rjBox,8); stroke(rjBox,Theme.accA,1).Transparency=.5; rjBox.Parent=oR
...
-- ===== Row: Auto Loop =====
local _, aL, aR = newRow(46); aL.Text="Auto loop"
  ...
  if Settings.autoLoop and not looping then
    ...
    task.spawn(function()
      while Settings.autoLoop do
        if #waypoints==0 then task.wait(0.15)
        else
...
-- restore angka/toggle setelah re-exec
