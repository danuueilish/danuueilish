...
-- Section "Manual" (waypoints + auto loop + dance 3x/8stud + auto respawn/kill + auto rejoin)

----------------------------------------------------------------
-- Ambil container "Manual" dari UI utama
----------------------------------------------------------------
...
if not container then
  -- fallback kalau ui_main.lua belum nambah "Manual"
  container = UI.NewSection(UI.Tabs.Mount, "Manual")
end
...
----------------------------------------------------------------
-- Row helper (label kiri 150px, kontrol kanan sisanya)
----------------------------------------------------------------
local function newRow(height)
  local row=Instance.new("Frame")
  row.BackgroundColor3=Theme.card
  row.Size=UDim2.new(1,-4,0,height or 50)
  ...
  local lay=Instance.new("UIListLayout", right)
...
----------------------------------------------------------------
-- CARD: List Waypoints + tombol Set / Delete
----------------------------------------------------------------
do
  ...
  listCard.Size=UDim2.new(1,-4,0,150)
  listCard.Parent=container
  ...
  local function refreshWP()
    ...
    for i,pos in ipairs(waypoints) do
      ...
      tp.Size=UDim2.new(0,44,1,0)
      tp.Position=UDim2.new(1,-94,0,0)
      ...
      del.Position=UDim2.new(1,-46,0,0)
...
----------------------------------------------------------------
local _, dLeft, dRight = newRow(46)
dLeft.Text = "Delay"
local delayBox = Instance.new("TextBox")
delayBox.Size=UDim2.new(0,140,1,0)
...
killBtn.Size=UDim2.new(0,200,1,0)
...
----------------------------------------------------------------
-- Row: Dance 3x/8stud + Auto Rejoin + Delay RJ
----------------------------------------------------------------
local _, gLeft, gRight = newRow(46)
gLeft.Text = "Opsi"

local danceBtn = Instance.new("TextButton")
danceBtn.Size=UDim2.new(0,200,1,0)
...
rjBtn.Size=UDim2.new(0,160,1,0)
...
rjBox.Size=UDim2.new(0,140,1,0)
...
----------------------------------------------------------------
-- Row: Auto Loop toggle
----------------------------------------------------------------
local _, aLeft, aRight = newRow(46)
aLeft.Text = "Auto loop"
local loopBtn = Instance.new("TextButton")
loopBtn.Size=UDim2.new(0,180,1,0)
      ...
      while Settings.autoLoop do
        if #waypoints==0 then task.wait(0.15) else
          ...
          for i=1,#waypoints do
            ...
            checkpointDance(pos)
            local t0=tick()
            while tick()-t0 < d and Settings.autoLoop do task.wait(0.05) end
...
-- apply ulang nilai tersimpan ke UI
do
  delayBox.Text = tostring(Settings.loopDelay or 3)
  rjBox.Text    = tostring(Settings.autoRJDelay or 5)
